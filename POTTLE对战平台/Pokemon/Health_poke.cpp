...
Health_poke::Health_poke(Health_poke_race race_) :Pokemon()//构造函数
{
	this->type = Poke_type::health_type;//精灵种类
	this->race = race_;//精灵种族
	this->hp += HEALTH_INC;//特征属性
}

std::string Health_poke::attack_to(Pokemon* a)//攻击函数
{
	//图形化界面
	for (int i = 0; i < (ATTACK_LASTING / BLINK_INTERVAL) / 2; i++)
	{
		...
		//攻击动作
		putimage(400, 150, &rocks_image_cover, SRCPAINT);
		putimage(400, 150, &rocks_image, SRCAND);
	...
	send_string += get_race() + " attacking with rocks to ";
	std::cout << get_race() << " attacking with rocks to ";
	send_string += a->attack_from(this->attack, 0, 0, 0);//特性攻击
	return send_string;
}

void Health_poke::level_inc()//等级上升
{
	this->level++;
...
std::string Health_poke::get_race()//获得种族
{
	return to_string(this->race);
}

std::string Health_poke::show_temp_info()const//返回精灵战斗临时信息
{
	std::string ret;
