...
Agility_poke::Agility_poke(Agility_poke_race race_) :Pokemon()//构造函数
{
	this->type = Poke_type::agility_type;//精灵种类
	this->race = race_;//精灵种族
	this->attack_interval -= INTERVAL_DEC;//特征属性
}

std::string Agility_poke::attack_to(Pokemon* a)//攻击函数
{
	//图形化界面
	for (int i = 0; i < (ATTACK_LASTING / BLINK_INTERVAL) / 2; i++)
	{
		...
		//攻击动作
		putimage(400, 360, &lightning_image_cover, SRCPAINT);
		putimage(400, 360, &lightning_image, SRCAND);
	...
	send_string += get_race() + " attacking with high speed to ";
	std::cout << get_race() << " attacking with high speed to ";

	send_string += a->attack_from(this->attack, 0, 0, 1);//特性攻击
	return send_string;
}

void Agility_poke::level_inc()//等级上升
{
	this->level++;
	...
	this->attack_interval -= INTERVAL_DOWN;//特性属性增加幅度大
	if (this->attack_interval <= MIN_INTERVAL)//攻击速度有上限
		this->attack_interval = MIN_INTERVAL;
}

std::string Agility_poke::show_info()//返回精灵信息
{
	std::string ret;
	ret += get_race();
	ret += "  Agility type";
...
std::string Agility_poke::get_race()//获得种族
{
	return to_string(this->race);
}

std::string Agility_poke::show_temp_info()const//返回精灵战斗临时信息
{
	std::string ret;
